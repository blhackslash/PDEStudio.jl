...
# Test MakiePlotting
function doTestSimulation(params::ParamDictType)
    
    x1 = collect(0.:params["dx"]:1.)
    u1 = copy(x1)
    x2 = collect(0.:params["dx"]:1.)
    u2 = x2.^2
    
    x = [x1,x2]
    if params["method"] == "test1"
        u = [u1,u2]
    elseif params["method"] == "test2"
        u = [params["a"].*u1, u2]
    end
    t = [0.,1.]

    sim_data = SimData1D(x,u,t,params)

    if params["method"] == "test1"
        sim_data.stats["Norm"] = [0,1]
        sim_data.stats["time"] = [1,10]
        sim_data.stats["test1"] = [1,1]
    elseif params["method"] == "test2"
        sim_data.stats["Norm"] = params["a"].*[0,1]
        sim_data.stats["time"] = [5,4]
        sim_data.stats["test2"] = [2,2]
    end 
